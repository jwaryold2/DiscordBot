...
import APIs.InsultAPIResponse;
import APIs.InsultAPI;
	...
	public void onMessageReceived(MessageReceivedEvent event) {
		...
		if (args[0].equals(insultP)) {
			try {
				...
				InsultAPI ins = GSON.fromJson(body, InsultAPI.class);
				for (int i = 0; i < ins.IR.length;i++) {
				if (mentioned.size()>0) {
					event.getChannel().sendMessage("<@!" + mentioned + ">" + ins.IR[i].insult).queue();
				} else {
					event.getChannel().sendMessage(ins.IR[i].insult).queue();
				}
				}
			} catch (Exception e) {
				System.err.println(e);
